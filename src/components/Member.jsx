    ...
    >
      ...
      <div className="relative mx-auto max-w-5xl pt-16 sm:px-6">
        <div className="sm:rounded-6xl bg-slate-50 pt-px">
          {member.logo && (
            <div className="relative mx-auto -mt-16 h-44 w-44 overflow-hidden rounded-full bg-slate-200 md:float-right md:h-64 md:w-64 md:[shape-outside:circle(40%)] lg:mr-5 lg:h-60 lg:w-60">
              <Image
          ...
          <div className="px-4 py-10 sm:px-10 sm:py-16 md:py-20 lg:px-20 lg:py-8">
            <SectionHeading number={member.id.toString()} id="author-title">
              Our Vision
            </SectionHeading>
            <div className="mb-6 border-l-2 border-blue-500 p-6">
              <p className="mb-4 font-display text-3xl tracking-tight text-blue-900">
                {member.vision || member.mission}
              </p>
              <h2 className="text-heading mb-4 text-3xl font-semibold leading-tight  tracking-tighter md:text-5xl">
                {member.name}
              </h2>
            </div>
            ...
            <p className="text-coolGray-400 mb-8 text-base md:text-lg">
            ...
            {member.cover && (
              <>
                ...
                <p className="text-coolGray-400 mb-8 text-base md:text-lg">
                  <span>{member.primaryCaption}</span>
                </p>
              </>
            )}
            ...
            {member.secondaryImage && (
              <>
                <div className="mb-4 max-w-max overflow-hidden rounded-md lg:py-8">
                  <img src={member.secondaryImage} alt="" />
                </div>
                <p className="text-coolGray-400 mb-8 text-base md:text-lg">
            ...
            {member.services && <TableOfContents services={member.services} />}
            {member.phoneNumbers && (
              <p className="border-coolGray-100 mb-10 border-b pb-10 text-base text-slate-700 md:text-lg">
                <span>
                  {member.conclusion} Contact us on{' '}
                  <a href={`tel:${member.phoneNumbers[0]}`}>
                    {member.phoneNumbers[0]}
                  </a>{' '}
                  {member.phoneNumbers[1] && `and `}{' '}
                  <a href={`tel: ${member.phoneNumbers[1]}`}>
                    {member.phoneNumbers[1]}
                  </a>{' '}
            ...
            <div className="flex items-center justify-center">
              <a
                className="border-coolGray-200 text-coolGray-300 hover:border-coolGray-300 hover:bg-coolGray-100 hover:text-coolGray-400 mr-4 inline-flex items-center justify-center rounded-md border bg-white py-2 px-4 shadow-md transition duration-200"
                href="#"
              >
                <svg
                  width="20"
                ...
                <span className="hover:text-coolGray-600 ml-2 text-sm text-blue-600">
                  Copy Link
                </span>
              </a>
              {member.socials?.facebook && (
                <a
                  className="border-coolGray-200 text-coolGray-500 hover:bg-coolGray-100 hover:text-coolGray-600 mr-2 inline-flex h-9 w-9 items-center justify-center rounded-md border bg-white shadow-md transition duration-200"
              ...
              {member.socials?.instagram && (
                <a
                  className="border-coolGray-200 text-coolGray-500 hover:bg-coolGray-100 hover:text-coolGray-600 inline-flex h-9 w-9 items-center justify-center rounded-md border bg-white shadow-md transition duration-200"
