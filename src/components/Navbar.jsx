...
const sections = [
  { id: 'member-1', title: 'Lifeline/Childline' },
  { id: 'member-2', title: 'Serenity Wellness Center' },
  { id: 'member-3', title: 'Strong Minds' },
  { id: 'member-4', title: 'Walk With Me Foundation' },
  { id: 'member-5', title: 'Mind Care' },
]
      ...
      <div className="hidden sm:flex sm:h-32 sm:justify-center sm:border-b sm:border-slate-200 sm:bg-white/95 sm:[@supports(backdrop-filter:blur(0))]:bg-white/80 sm:[@supports(backdrop-filter:blur(0))]:backdrop-blur">
        ...
        >
          ...
          {sections.map((section, sectionIndex) => (
            <li
              key={section.id}
              className="flex text-center [counter-increment:section]"
            >
              <Link
                href={`#${section.id}`}
                className={clsx(
